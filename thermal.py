...
MINER_PROFILES = {
    ...
    # Single-chip NerdAxe (similar to BitAxe)
    ...
    # Multi-chip NerdQAxe (4 chips, higher power/heat)
    'NerdQAxe': FrequencyProfile(
        min_freq=400,
        max_freq=575,      # Slightly lower max due to heat density
        default_freq=485,
        optimal_temp=58.0,  # Lower target due to multiple chips
        warning_temp=63.0,
        critical_temp=66.0, # More conservative for multi-chip
        max_chip_temp=72.0,
        temp_hysteresis=2.0,
        freq_step=10
    ),
    # NerdOctaxe (6-8 chips, even higher power/heat)
    'NerdOctaxe': FrequencyProfile(
        min_freq=400,
        max_freq=550,      # Lower max due to high heat density
        default_freq=475,
        optimal_temp=55.0,  # Lower target for many chips
        warning_temp=60.0,
        critical_temp=64.0, # Very conservative for 6-8 chips
        max_chip_temp=70.0,
        temp_hysteresis=2.0,
        freq_step=10
    ),
